...
The licensing-exam results deserve the same caution. A human physician's competence can be established with a finite exam because of what we assume about human generalization and supervision. A model could answer millions of questions well and still fail bizarrely in a rare region of the input space. The regulatory question is not whether the model *knows medicine* — it is whether it *behaves predictably enough for a specified use*.
